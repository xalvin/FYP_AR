...
public class CustomItemizedOverlay extends ItemizedOverlay<OverlayItem> {
	 ...
	 * selection etc
	 */
	private EfficientList<GeoObj> itemList;
	private GeoGraph mygraph;

	public EfficientList<GeoObj> getItemList() {
		if (itemList == null)
			itemList = new EfficientList<GeoObj>();
		return itemList;
	}
	...
	public int size() {
		...
		if (itemList.myLength != mygraph.length()) {
			...
			for (int i = 0; i < nodes.myLength; i++) {
				itemList.add(new GeoObj(nodes.get(i)));
			}
			/*
